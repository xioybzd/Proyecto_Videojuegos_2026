...
export default function CelularScreen() {
  ...
  const { capitulosCompletados, agregarPista, marcarConversacionCelularVista } = useContext(GameContext);
  ...
  const conversacionActivaId = yaTerminoCap6
    ? 'cap6'
    : yaTerminoCap5
      ? 'cap5'
      : yaTerminoCap4
  ...
  const cerrarCelular = async () => {
    ...
    if (!yaTerminoCap4 && !yaTerminoCap5 && !yaTerminoCap6) {
      ...
      marcarConversacionCelularVista(conversacionActivaId);
    }
  ...
  const headerStatus = yaTerminoCap6
    ? 'nota recuperada hoy'
    : yaTerminoCap5
      ? 'conversación de hace 1 semana'
      : yaTerminoCap4
        ? 'conversación de hace 2 semanas'
        : 'conversación de hace 3 semanas';
